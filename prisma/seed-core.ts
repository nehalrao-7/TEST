...
// Seeds a fresh database with Game6's starting content. Idempotent for the
// non-program tables (it clears + recreates them); programs are upserted by slug.
export async function seed(prisma: PrismaClient) {
  await prisma.season.deleteMany();
  await prisma.season.create({
    data: { label: "Spring", year: 2026, registrationStatus: "OPEN", headline: null, isActive: true },
  });

  const programRecords: Record<string, string> = {};
  for (const p of programs) {
    const rec = await prisma.program.upsert({ where: { slug: p.slug }, update: p, create: p });
  ...
  // Sample recurring classes (weekday: 0=Sun..6=Sat).
  const hsTue = await prisma.classSession.create({
    data: { name: "High School Practice", weekday: 2, startTime: "18:00", endTime: "20:00", capacity: 24, ageHint: "High school", programId: programRecords["g6bl"], courtId: court1.id },
  });
  await prisma.classSession.create({
    data: { name: "High School Practice", weekday: 4, startTime: "18:00", endTime: "20:00", capacity: 24, ageHint: "High school", programId: programRecords["g6bl"], courtId: court1.id },
  });
  const elemWed = await prisma.classSession.create({
    data: { name: "Elementary Practice", weekday: 3, startTime: "17:00", endTime: "18:30", capacity: 20, ageHint: "Elementary", programId: programRecords["junior-ball"], courtId: court2.id },
  });
